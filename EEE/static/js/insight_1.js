...
// 데이터를 불러옵니다.
d3.csv("/static/data/mergedata_fin.csv").then(data => {
  ...
  function updateVisualization() {
    ...
    svg.append("path")
      ...
      .attr('y', height + 50)  // X 축 아래에 위치
      .attr('text-anchor', 'middle')  // 텍스트 가운데 정렬
      .text('Year');  // X 축 이름 텍스트
   
   svg.append('text')
      .attr('class', 'y-axis-label')  // CSS 클래스 추가 (스타일링 용)
      .attr('x', -50)  // 세로 가운데 정렬 및 90도 회전
      .attr('y', 50)  // Y 축 왼쪽에 위치
      .attr('text-anchor', 'middle')  // 텍스트 가운데 정렬
      .text('(%)');  // Y 축 이름 텍스트

    console.log(newdata5[0]['class']);
    var countryClass = newdata5[0]['class'];
    let groupColor;
    if (countryClass === 'underdeveloped country') {
      groupColor = '#2CA02C';
    } else if (countryClass === 'developing country') {
      groupColor = '#1F77B4';
    } else if (countryClass === 'Developed country') {
      groupColor = '#FF7F0E';
    }
...
svg.append("path")
    ...
    tooltip
      ...
      .style('top', (d3.pointer(event)[1] + 120) + 'px'); // d3.pointer(event) 이벤트가 일어난 위치 정보를 받아올 수 있음
    }
